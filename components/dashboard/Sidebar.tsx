...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, userRole }) => {
  ...
  const getPatientMenuItems = (): MenuItem[] => {
    ...
    // Adicionar item de Avaliações apenas se houver alguma avaliação habilitada
    const avaliacoesChildren: MenuItem[] = [];

    if (user?.tdah_screening_enabled) {
      avaliacoesChildren.push(
        { label: 'Triagem TDAH', path: '/dashboard/patient/tdah-screening', icon: '🧠' },
        { label: 'Meus Resultados', path: '/dashboard/patient/tdah-screening', icon: '📊' }
      );
    }

    if (avaliacoesChildren.length > 0) {
      items.push({
        label: 'Avaliações',
        icon: '🧠',
        children: avaliacoesChildren
      });
    }
  ...
  const menuItems = userRole === 'patient' ? getPatientMenuItems() : getTherapistMenuItems();
  ...
  useEffect(() => {
    const currentMenuItems = userRole === 'patient' ? getPatientMenuItems() : getTherapistMenuItems();
        ...
        {menuItems.map((item, index) => {
                  ...
                  >
                    {item.children.map((child, childIndex) => (
                      <Link
                        key={childIndex}
                        to={child.path || '#'}
                        className={`flex items-center px-4 py-2 transition-colors rounded-sm ${isActive(child.path)
          ...
          return (
            <Link
              key={index}
              to={item.path || '#'}
              className={`flex items-center px-4 py-3 transition-colors rounded-sm ${isActive(item.path)
                ? 'bg-teal-600 text-white border-l-4 border-teal-400 font-medium'
                : 'text-slate-300 hover:bg-slate-800 hover:text-white'
                }`}
              title={!isOpen ? item.label : undefined}
