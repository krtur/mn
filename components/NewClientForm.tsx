...
const NewClientForm: React.FC<NewClientFormProps> = ({ 
  ...
  const { createRequest } = useNewClientRequests();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Enviando solicitação para terapeuta:', therapistId);
      ...
      if (!result.success) {
        console.error('Falha no envio da solicitação:', result.error);
      ...
      setError('Não foi possível enviar sua solicitação. Verifique sua conexão com a internet e tente novamente.');
    } finally {
