...
interface DirectRestApiFormProps {
  onSuccess?: () => void;
  onCancel?: () => void;
...
const DirectRestApiForm: React.FC<DirectRestApiFormProps> = ({ 
  onSuccess, 
  onCancel, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Enviando solicitação via API REST direta');
      console.log('Dados:', { name, email, phone, therapist_id: therapistId });
      
      // URL do Supabase
      const supabaseUrl = 'https://ygfxloachqjeslciyunr.supabase.co';
      
      // Chave anônima do Supabase (a mesma usada no arquivo services/supabase.ts)
      ...
      console.log('Configuração:', { supabaseUrl, keyLength: supabaseAnonKey.length });
      
      // Usar fetch diretamente para contornar RLS
      const response = await fetch(`${supabaseUrl}/rest/v1/new_client_requests`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className={`${isModal ? 'p-6' : 'p-8 card-premium rounded-xl shadow-lg max-w-md mx-auto'}`}>
      <h2 className="text-2xl font-bold text-slate-900 mb-6">
        {preselectedTherapist ? 'Agende Sua Sessão' : 'Agende Sua Primeira Sessão'}
      </h2>
...
export default DirectRestApiForm;
